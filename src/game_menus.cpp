...
void GameMenu::draw( SDL_Surface *surface, Uint8 alpha ) const
{
	...
	font.blit(surface, 0, surface->h - font.height(font_sprites[1]), "v0.4-b", font_sprites[1], Font::MAJUSCULE, Font::LEFT, alpha);
	
	x = surface->w - 1;
	y = surface->h - font.height(font_sprites[1]) * 2;
	
	font.blit(surface, x, y, "game: Carl \"Mindless\" Reinke", font_sprites[1], Font::MAJUSCULE, Font::RIGHT, alpha);
